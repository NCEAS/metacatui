...
define([
  ...
  describe("ViewfinderModel Test Suite", () => {
    const state = cleanState(() => {
      ...
      const autocompleteSpy = sandbox
        .stub(model.geocoderSearch, "autocomplete")
        .returns([]);
      const geocodeSpy = sandbox
        .stub(model.geocoderSearch, "geocode")
        .returns([]);
      const predictions = [
        new Prediction({
    ...
    describe("autocomplete search", () => {
      ...
      it("sets predictions from autocomplete search", async () => {
        ...
        expect(state.model.get("predictions")[0].get("description")).to.equal(
          "Some Other Location",
        );
      });
    ...
    describe("selecting a prediction", () => {
      ...
      it("shows permissions error if geocoder request is denied", async () => {
      ...
      it("shows 'no results' message if geocoder throws some other error", async () => {
        state.geocodeSpy.callsFake(() => {
          const error = new Error("Some other error");
          throw error;
        });
