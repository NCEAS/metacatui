...
define([
  ...
  describe("GoogleMapsGeocoder Test Suite", () => {
    ...
    it("calls the Google Maps API with the provided place ID", async () => {
      ...
      expect(
        state.googleMapsGeocoder.geocoder.geocode.getCall(0).firstArg,
      ).to.deep.equal({ placeId: "abc123" });
    });

    it("returns results from the Google Maps API", async () => {
      const response = await state.googleMapsGeocoder.geocode(state.prediction);
    ...
    it("throws an error if the Google Maps API throws", async () => {
      ...
      state.sandbox
        .stub(state.googleMapsGeocoder.geocoder, "geocode")
        .rejects();

      expect(async () => state.googleMapsGeocoder.geocode(state.prediction)).to
        .throw;
    });
  });
});
