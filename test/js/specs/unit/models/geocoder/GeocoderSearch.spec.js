...
define([
...
], (cleanState, GeocoderSearch, Prediction, GeocodedLocation) => {
  ...
  describe("GeocoderSearch Test Suite", () => {
    const state = cleanState(() => {
      ...
      sandbox.stub(geoSearch.googleMapsAutocompleter, "autocomplete").returns([
        new Prediction({
      ...
      sandbox.stub(geoSearch.googleMapsGeocoder, "geocode").returns([
    ...
    it("searches by Google Maps Place ID and returns geocoded locations", async () => {
      ...
      const geoLocs = await state.geoSearch.geocode(
        new Prediction({
          description: "some desc",
          googleMapsPlaceId: "somePlaceId",
        }),
