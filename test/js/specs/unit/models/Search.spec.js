    ...
    describe("Constructing search queries", function () {
      ...
      describe("The default Data Catalog query", function () {
        ...
        it("should get metadata only", function () {
          expect(query.indexOf("formatType:METADATA")).to.be.at.least(
            0,
            "metadata filter is not in the query string ",
          );
        });

        it("should exclude portals and collections", function () {
          expect(
            query.indexOf(" -formatId:*dataone.org%5C%2Fcollections* "),
          ).to.be.at.least(
            0,
            `collections filter is not in the query: (${query})`,
          );
          expect(
            query.indexOf(" -formatId:*dataone.org%5C%2Fportals* "),
          ).to.be.at.least(0, `portals filter is not in the query: (${query})`);
        });

        it("should exclude obsoleted metadata", function () {
          expect(query.indexOf("-obsoletedBy:*")).to.be.at.least(
            0,
            "obsoletedBy is not in the query",
          );
        });
      });
      ...
      describe("Adding filters", function () {
        ...
        it("should add an All filter", function () {
          ...
          query
            ...
            .should.be.at.least(
              0,
              `filter is not in the query string: (${query})`,
            );
        });
        ...
        it("should add a creator filter", function () {
          ...
          let query = search.getQuery();
          query
            .indexOf(search.fieldNameMap.creator + ":%22Homer%20Simpson%22")
        ...
        it("should add a location filter", function () {
          ...
          let query = search.getQuery();
          query
            .indexOf(search.fieldNameMap.spatial + ":%22A%20place%22")
            .should.be.at.least(
              0,
              `filter is not in the query string: (${query})`,
            );
        ...
        it("should add a begin and end date filter", function () {
          ...
          query
            .indexOf(
              "beginDate:[1800-01-01T00:00:00Z TO *]" +
                " AND endDate:[* TO 2015-12-31T00:00:00Z]",
            )
            .should.be.at.least(
              0,
              `filter is not in the query string: (${query})`,
        ...
        it("escape special characters", function () {
          ...
          console.log(query);
          let expectedQuery =
            "%5C%22%5C%2B%20%5C-%20%20%5C%7C%5C%7C%20!%20%5C(%20%5C)%20%5C%7B%20%5C%7D%20%5C%5B%20%5C%5D%20%5C%5E%20%5C%22%20%5C~%20*%20%5C%3F%20%5C%3A%20%5C%2F%5C%22";
          query
            .indexOf(expectedQuery)
            .should.be.at.least(
              0,
              `filter is not in the query string: (${query})`,
            );
