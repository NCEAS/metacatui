    ...
    describe("Sending the default Data Catalog query", function () {
      it("should get the correct results", function (done) {
        ...
        solrResults.setfields(
          "id,seriesId,title,origin,pubDate,dateUploaded,abstract,resourceMap,beginDate,endDate,read_count_i,geohash_9,datasource,isPublic,documents,sem_annotation,northBoundCoord,southBoundCoord,eastBoundCoord,westBoundCoord,formatType,formatId",
        );
        ...
        solrResults.once("reset", function () {
          ...
          solrResults.pluck("obsoletedBy").every((r) => r === null).should.be
            .true;
          solrResults.pluck("formatType").every((r) => r === "METADATA").should
            .be.true;
          solrResults.pluck("formatId").every((r) => {
            let e = new RegExp(/dataone\.org\/(portals|collections)/, "g");
            return e.test(r);
          }).should.be.false;
          solrResults.pluck("archived").every((r) => r === false).should.be
            .true;
