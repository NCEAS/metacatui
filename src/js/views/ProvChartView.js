define([
  ...
  var ProvChartView = Backbone.View.extend({
    initialize: function (options) {
      ...
      //For Sources charts
      if (
        (!this.derivations && this.sources) ||
        (this.editModeOn && this.type == "sources")
      ) {
        this.type = "sources";
        ...
        _.each(this.sources, function (model) {
          if (model.getType() == "program") programs.push(model);
      ...
      if (
        (!this.sources && this.derivations) ||
        (this.editModeOn && this.type == "derivations")
      ) {
      ...
      if (this.context.getType() == "program" && this.type == "derivations") {
        ...
        this.context.getType() == "program" &&
        this.type == "sources"
      ) {
        this.title = this.numProvEntities + " inputs";
      } else this.title = this.numProvEntities + " " + this.type;
    ...
    render: function () {
      ...
      if (
        ...
        (this.editModeOn && this.context.getType() != "program")
      ) {
        this.$el.append(
      ...
      _.each(
        ...
        function (entity, i) {
          ...
          if (entity.getType() == "Package")
            view.$el.append(
              view.createNode(
                entity,
                position,
                _.find(entity.get("members"), function (member) {
                  return member.get("formatType") == "METADATA";
                }),
              ),
            );
          else {
            ...
            if (Array.isArray(metadataID)) metadataID = metadataID[0];

            if (metadataID) {
              //The metadata doc for this object may be in the same package as the context of this prov chart
              metadata = view.dataPackage.find(function (member) {
                return member.get("id") == metadataID;
              });
            }
            ...
            if (!metadata) {
              ...
              _.each(view.dataPackage.get("relatedModels"), function (model) {
                potentialMatch = _.find(
                  model.get("members"),
                  function (member) {
                    return member.get("id") == metadataID;
                  },
                );
                if (potentialMatch) metadata = potentialMatch;
              });
            }
          ...
          if (entity.getType() == "program") programPosition++;
          else position++;
        },
        this,
      );
      ...
      if (this.editModeOn) {
        ...
        if (this.context.getType() != "program" && this.numPrograms == 0) {
          var programNode = this.createEditorNode(
            "program",
            this.context.get("id"),
            programPosition,
          );
        ...
        var dataNode = this.createEditorNode(
          "data",
          this.context.get("id"),
          position,
        );
        ...
        if (this.type == "sources") this.numSources++;
        if (this.type == "derivations") this.numDerivations++;
    ...
    createNode: function (provEntity, position, metadata) {
      ...
      if (provEntity.type == "DataONEObject") {
        ...
        (icon = "icon-folder-open"), (type = "package");
      }
              ...
              $(e.target)
                ...
                .on("click", function (evt) {
                  ...
                  view.removeProv(
                    ...
                    evt.target.parentNode.getAttribute("class"),
                  );
                });
            },
            // mouseleave action
          ...
          $(svg).hover(
            ...
            function (e) {
              ...
              deleteIcon.on("click", function (evt) {
                ...
                // This will prevent the node popover from displaying for this node when the delete icon is clicked.
                evt.stopPropagation();
                var dataId = $(evt.target)
                  .parent()
                  .parent()
                  .find("polygon")
                  .attr("data-id");
                var nodeClass = $(evt.target)
                  .parent()
                  .parent()
                  .find("polygon")
                  .attr("class");
      ...
      if (provEntity.getType() == "image") {
        $(nodeEl).css(
          ...
          "url('" +
            view.serviceUrl +
            encodeURIComponent(provEntity.get("id")) +
            "')",
        );
      }
      //Create an icon inside the node for other format types
      else {
        var iconEl = $(document.createElement("i")).addClass(icon + " icon");
      ...
      var createLink = true;
      if (
        provEntity.get("id") == MetacatUI.appModel.get("pid") ||
      ...
      //The title
      var titleEl = $(document.createElement("span")).append(
      ...
      //The View link
      var arrowIcon = $(document.createElement("i")).addClass(
      ...
      if (provEntity.getType() == "image") {
        var img = $(document.createElement("img"))
          .attr(
            "src",
            view.serviceUrl + encodeURIComponent(provEntity.get("id")),
          )
          .addClass("thumbnail");
      ...
      var view = this,
        popoverTriggerEl =
          provEntity.getType() == "program" ? $(nodeEl).add(g) : nodeEl;
    ...
    collapseNodes: function (scroll) {
      ...
      else this.$el.height(this.height);
    ...
    selectProvEntities: function (e) {
      ...
      // data edit icon.
      var title = null;
      var label = "Choose from files in this dataset: ";
      var helpText = null;
      var provInfoURL = MetacatUI.appModel.get("provenanceInfoURL");
      var helpLink = provInfoURL
        ? $(document.createElement("a"))
            .attr("href", provInfoURL)
            .attr("target", "_blank")
            .text("Learn more about data provenance")
            .append(
              $(document.createElement("i")).addClass(
                "icon icon-on-right icon-long-arrow-right",
              ),
            )
        : null;
      ...
      var isContextAProgram = this.context.getType() == "program";
      ...
      if (this.type == "sources") {
        if (isProgram) {
          title =
            "Add the program that generated " + this.context.get("fileName");
          helpText = $(document.createElement("p"))
            .addClass("help-text subtle alert")
            .text("Describe the origin and processing history of  ")
            .append(
              $(document.createElement("code"))
                .addClass("filename")
                .text(this.context.get("fileName")),
            )
            .append(
              " by selecting which program, script, or code created it. ",
            );
        } else if (isContextAProgram) {
        ...
        if (isProgram) {
          title = "Add the program that used " + this.context.get("fileName");
          helpText = $(document.createElement("p"))
            .addClass("help-text subtle alert")
            .text("Describe the processing history of  ")
            .append(
              $(document.createElement("code"))
                .addClass("filename")
                .text(this.context.get("fileName")),
          ...
          title = "Add derived data for " + this.context.get("fileName");
          helpText = $(document.createElement("p"))
            .addClass("help-text subtle alert")
            .text("Describe the processing history of ")
            .append(
            ...
            .append(
              " by selecting which data object were created by or output by it. ",
            );
        } else {
          title = "Add derived data for " + this.context.get("fileName");
          helpText = $(document.createElement("p"))
        ...
        if (this.type == "sources") {
          ...
          if (entityType == "program") {
            ...
            _.each(view.sources, function (model) {
              if (model.getType() == "data") {
                ...
                if (addMember != null) {
                  model.set(
                    "provDerivations",
                    _.union(model.get("provDerivations"), [addMember]),
                  );
                  addMember.set(
                    "provSources",
                    _.union(addMember.get("provSources"), [model]),
                  );
                }
              }
            });
            ...
            if (addMember != null) {
              addMember.set(
                "provDerivations",
                _.union(addMember.get("provDerivations"), [this.context]),
              );
            }
          } else {
            // source chart
            ...
            if (this.context.getType() == "program") {
              ...
              progMember.set(
                "provSources",
                _.union(progMember.get("provSources"), [addMember]),
              );
              addMember.set(
                "provDerivations",
                _.union(addMember.get("provDerivations"), [progMember]),
              );
              ...
              this.context.set(
                "provSources",
                _.union(this.context.get("provSources"), [addMember]),
              );
              ...
              _.each(view.programs, function (thisProgram) {
                ...
                thisProgram.set(
                  "provSources",
                  _.union(thisProgram.get("provSources"), [addMember]),
                );
              });
            }
          }
          ...
          if (entityType == "program") {
            ...
            _.each(view.derivations, function (model) {
              if (model.getType() == "data") {
                ...
                if (addMember != null) {
                  model.set(
                    "provSources",
                    _.union(model.get("provSources"), [addMember]),
                  );
                  addMember.set(
                    "provDerivations",
                    _.union(addMember.get("provDerivations"), [model]),
                  );
                }
              }
            });
            ...
            if (addMember != null) {
              addMember.set(
                "provSources",
                _.union(addMember.get("provSources"), [this.context]),
              );
            }
          } else {
            // derivations chart
            ...
            if (this.context.getType() == "program") {
              ...
              progMember.set(
                "provDerivations",
                _.union(progMember.get("provDerivations"), [addMember]),
              );
              ...
              this.context.set(
                "provDerivations",
                _.union(this.context.get("provDerivations"), [addMember]),
              );
              ...
              _.each(view.programs, function (thisProgram) {
                ...
                thisProgram.set(
                  "provDerivations",
                  _.union(thisProgram.get("provDerivations"), [addMember]),
                );
              });
            }
          }
    ...
    removeProv: function (pidToRemove, classNames) {
      ...
      if (typeof classNames == "string") {
        entityType = _.contains(classNames.split(" "), "program")
          ? "program"
          : "data";
      } else {
      ...
      if (this.type == "sources") {
        ...
        if (entityType == "program") {
          ...
          _.each(view.sources, function (model) {
            if (model.getType() == "data") {
              ...
              if (removeMember != null) {
                model.set(
                  "provDerivations",
                  _.reject(model.get("provDerivations"), function (item) {
                    return item.get("id") == pidToRemove;
                  }),
                );
                removeMember.set(
                  "provSources",
                  _.reject(removeMember.get("provSources"), function (item) {
                    return item.get("id") == dataPid;
                  }),
                );
              }
            }
          ...
          if (removeMember != null) {
            removeMember.set(
              "provDerivations",
              _.reject(removeMember.get("provDerivations"), function (item) {
                return item.get("id") == memberPid;
              }),
            );
          }
        } else {
          ...
          if (this.context.getType() == "program") {
            ...
            progMember.set(
              "provSources",
              _.reject(progMember.get("provSources"), function (item) {
                return item.get("id") == pidToRemove;
              }),
            );
            removeMember.set(
            ...
            _.each(progMember.get("provDerivations"), function (model) {
              if (model.getType() == "data") {
                ...
                removeMember.set(
                  "provDerivations",
                  _.reject(
                    removeMember.get("provDerivations"),
                    function (item) {
                      return item.get("id") == dataPid;
                    },
                  ),
            ...
            this.context.set(
              "provSources",
              _.reject(this.context.get("provSources"), function (item) {
                return item.get("id") == pidToRemove;
              }),
            );
            removeMember.set(
            ...
            _.each(view.programs, function (thisProgram) {
              ...
              thisProgram.set(
                "provSources",
                _.reject(thisProgram.get("provSources"), function (item) {
                  return item.get("id") == pidToRemove;
                }),
              );
            });
        ...
        if (entityType == "program") {
          ...
          _.each(view.derivations, function (model) {
            if (model.getType() == "data") {
              ...
              if (removeMember != null) {
                model.set(
                  "provSources",
                  _.reject(model.get("provSources"), function (item) {
                    return item.get("id") == pidToRemove;
                  }),
                );
                removeMember.set(
          ...
          if (this.context.getType() == "program") {
            ...
            progMember.set(
              "provDerivations",
              _.reject(progMember.get("provDerivations"), function (item) {
                return item.get("id") == pidToRemove;
              }),
            );
            // Also add the derived data item to each input of the program.
            ...
            this.context.set(
              "provDerivations",
              _.reject(this.context.get("provDerivations"), function (item) {
                return item.get("id") == pidToRemove;
              }),
            );
            ...
            _.each(view.programs, function (thisProgram) {
              ...
              thisProgram.set(
                "provDerivations",
                _.reject(thisProgram.get("provDerivations"), function (item) {
                  return item.get("id") == pidToRemove;
                }),
              );
            });
    ...
    addProvRel: function (dataPackage, subjectId, predicate, object) {
      var subjectMember = _.find(dataPackage.toArray(), function (member) {
        return member.get("id") == subjectId;
      });
      var objectMember = _.find(dataPackage.toArray(), function (member) {
        return member.get("id") == object;
      });
      ...
      if (!isSource && !isDerivation) {
        ...
        subjectMember.set(
          "provSources",
          _.union(subjectMember.get("provSources"), [objectMember]),
        );
        subjectMember.set(
          predicate,
          _.union(subjectMember.get(predicate), [objectMember]),
        );
      } else {
        ...
        subjectMember.set(
          "provDerivations",
          _.union(subjectMember.get("provDerivations"), [objectMember]),
        );
        subjectMember.set(
          predicate,
          _.union(subjectMember.get(predicate), [objectMember]),
    ...
    removeProvRel: function (dataPackage, subjectId, predicate, object) {
      ...
      if (!isSource && !isDerivation) {
        ...
        subjectMember.set(
          "provSources",
          _.reject(subjectMember.get("provSources"), objectMember),
        );
        subjectMember.set(
          predicate,
          _.reject(subjectMember.get(predicate), objectMember),
        );
      } else {
        // Its a derivation field
        // Also populate the 'derivations' accumulator field
        subjectMember.set(
          "provDerivations",
          _.reject(subjectMember.get("provDerivations"), objectMember),
        );
        subjectMember.set(
          predicate,
          _.reject(subjectMember.get(predicate), objectMember),
    ...
    setMemberAttr: function (dataPackage, pid, attr, value) {
      var thisMember = dataPackage.find(function (member) {
        return member.get("id") == pid;
      });
