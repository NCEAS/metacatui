...
define(["jquery", "underscore", "backbone", "d3"], function (
  ...
  var DonutChartView = Backbone.View.extend({
    ...
    render: function () {
      ...
      // sort or not
      var theData = donut.value(function (d) {
        return d.perc;
      });
      if (this.keepOrder) {
        theData = donut
      ...
      var arcs = vis
        ...
        .attr("transform", "translate(" + w / 2 + ", " + h / 2 + ")"); //center in the SVG element

      //Append an arc to each group
      arcs.append("svg:path").attr("class", "donut-arc").attr("d", arc);
      ...
      if (drawLabels) {
        ...
        var labelGroups = arcs.append("svg:g").attr("class", "donut-labels");

        //Keep track of how many labels we rotate
        var rotatedLabels = [],
          rotatedCounts = [],
          rotateWidth = 0.5; //The max arc width/length to attach straight labels to. Anything under this gets rotated.

        // Append a text label to each arc
        labelGroups
          .append("svg:text")
          .attr("transform", function (d, i) {
            //Calculate the label position based on arc centroid
            ...
            if (width < rotateWidth) {
              transform =
                ...
                this.transformX +
                "," +
                (this.transformY + 10 + rotatedLabels.length * 5) +
                ") rotate(30)";
          ...
          .attr("transform", function (d, i) {
            ...
            if (width < rotateWidth) {
              transform =
                ...
                (this.transformX - 10) +
                "," +
                (this.transformY + 20 + rotatedCounts.length * 5) +
                ") rotate(30)";
        ...
        if (rotatedLabels.length == 1)
          d3.select(rotatedLabels[0]).attr(
            "transform",
            "translate(" +
              (rotatedLabels[0].transformX + 5) +
              "," +
              (rotatedLabels[0].transformY - 5) +
              ")",
          );
        if (rotatedCounts.length == 1) {
          d3.select(rotatedCounts[0]).attr(
            "transform",
            "translate(" +
              rotatedCounts[0].transformX +
              "," +
              (rotatedCounts[0].transformY + 15) +
              ")",
          );
      ...
      if (
        (this.titleText || this.titleText !== undefined) &&
        (this.titleCount || this.titleCount !== undefined)
      ) {
        ...
        var attributes = count
          ...
          .attr(function (d) {
            return "transform", "translate(" + d.cx + "," + d.cy + ")";
          });
      }
      ...
      $(arcs).mouseover(function (e) {
        ...
        $(arc)
          ...
          .each(function (i, svg) {
            ...
            $(svg)
              .find("[class~=donut-arc]")
              .each(function (i, otherArc) {
          ...
          .each(function (i, svg) {
            ...
            $(svg)
              ...
              .each(function (i, otherArc) {
                ...
                classes = classes.replace("inactive", "").replace("active", "");
    ...
    formatDonutData: function (counts) {
      ...
      if (
        typeof counts[0] == "object" &&
        typeof counts[0].label != "undefined" &&
        typeof counts[0].count != "undefined"
      ) {
