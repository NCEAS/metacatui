define([
  ...
  const PortEditorSectionsView = Backbone.View.extend(
    /** @lends PortEditorSectionsView.prototype */ {
      ...
      /**
       * A reference to the currently active editor section. e.g. Data, Metrics,
       * Settings, etc.
      ...
      /**
       * The name of the active section when the view is first loaded. This is
       * retrieved from the router/URL
       * @type {string}
       */
    ...
    @type {Underscore.Template}
      ...
      /**
       * A jQuery selector for the elements that are links to the individual
       * sections
       * @type {string}
       */
      sectionLinks: ".portal-section-link",
      /**
       * A jQuery selector for the element that the section links should be
       * inserted into
       * @type {string}
       */
      sectionLinksContainer: ".section-links-container",
      /**
       * A jQuery selector for the element that a single section link will be
       * inserted into
       * @type {string}
       */
      sectionLinkContainer: ".section-link-container",
      /**
       * A jQuery selector for the element that the editor sections should be
       * inserted into
       * @type {string}
       */
      ...
      /**
       * A selector for link or tab elements that the user is allowed to
       * re-order, starting from the sectionLinksContainer
       * @type {string}
       */
      ...
      events: {
        ...
        // both keyup and keydown events are needed for limitLabelLength
        // function
      ...
      render() {
        ...
        const view = this;
        const linksContainer = view.el.querySelector(
          view.sectionLinksContainer,
        );
        const { sortableLinksSelector } = view;
        const sortableLinks = view.el.querySelectorAll(
          view.sectionLinksContainer + view.sortableLinksSelector,
        );
        const sortableLinksArray = Array.prototype.slice.call(sortableLinks, 0);
        ...
        try {
          if (pageOrder && pageOrder.length) {
            ...
            sortableLinksArray.sort((a, b) => {
              const aName = $(a).data("section-name");
              const bName = $(b).data("section-name");
              const aIndex = pageOrder.indexOf(aName);
              const bIndex = pageOrder.indexOf(bName);
              // If the label can't be found in the list of labels, place it at
              // the end
              if (bIndex === -1) {
                return +1;
              }
        ...
        if (this.activeSectionLabel) {
          ...
          // Reset the active section label, since it is only used during the
          // initial rendering
      ...
      renderAddSection() {
        ...
        const addSectionView = new PortEditorSectionView({
          model: this.model,
        ...
        this.$el
          .find(
            `${this.sectionLinkContainer}[data-section-name='${this.addPageLabel}'] a`,
          )
          .html("<i class='icon icon-plus'></i>")
          .attr("title", "Add a new page");

        // When a sectionOption is clicked in the addSectionView subview, the
        // "addNewSection" event is triggered.
      ...
      /**
       * Render all sections in the editor for each content section in the
       * Portal
       */
      ...
      renderDataSection() {
        try {
          ...
          const dataView = new PortEditorDataView({
            model: this.model,
      ...
      renderMetricsSection() {
        ...
        // If the metrics aren't hidden AND the metrics view was created
        // already, then show it
      ...
      /**
       * Adds or removes the metrics link depending on the 'hideMetrics' option
       * in the model.
       */
      toggleMetricsLink() {
        try {
      ...
      renderSettings() {
        ...
        const settingsView = new PortEditorSettingsView({
          model: this.model,
       ...
       * @param {boolean} [showSectionLabel] - If true, the editor section label
      ...
      updatePath(showSectionLabel) {
        ...
        const originalLabel = this.model.get("originalLabel") || "";
        ...
        // Add or replace the label part of the path with the new label. pathRE
        // matches "/label/section", where the "/section" part is optional
        ...
        // If there is an active section, and the showSectionLabel parameter is
        // true, add the section label to the path.
      ...
      /**
       * Manually switch to a section subview by making the tab and tab panel
       * active. Navigation between sections is usually handled automatically by
       * the Bootstrap library, but a manual switch may be necessary sometimes
       * @param {PortEditorSectionView} [sectionView] - The section view to
       * switch to. If not given, defaults to the activeSection set on the view.
       */
      switchSection(sectionView) {
        ...
        if (!activeSectionView) {
          ...
          // If we are defaulting to the first section, don't show the section
          // label in the URL
        ...
        // Never show the section label in the URL, since it messes with the
        // back/forward browser navigation. See #1364
        showSectionLabelInURL = false;
      ...
      handleSwitchSection(e) {
        ...
        if (sectionView) {
          ...
          // If the user clicks a link and is not near the top of the page (i.e.
          // on mobile), scroll to the top of the section content. Otherwise it
          // might look like the page hasn't changed
       ...
       * @param {boolean} isFocused - A boolean flag to enable focus on new
       * section link
       */
      addSectionLink(sectionView, menuOptions, isFocused) {
        try {
          ...
          // Find the "+" link to help determine the order in which we should
          // add links
          const addSectionEl = this.$(this.sectionLinksContainer).find(
       ...
       * link when a new section is added. Called on initial render and each
       * time a section is added, removed, shown, or hidden.
       */
      toggleRemoveSectionOption() {
        try {
          ...
          const removeSectionLinks = this.$(this.sectionLinkContainer).find(
            ".remove-section",
          );
          ...
          const content = $(document.createElement("div")).append(
            $(document.createElement("div")).append(
              $(document.createElement("p")).text(
                `Deleting this page will premanently remove it from this ${MetacatUI.appModel.get(
                  "portalTermSingular",
                )}.`,
              ),
            ),
      ...
      updateSectionLink(sectionView, menuOptions) {
        ...
        const sectionLink = this.createSectionLink(sectionView, menuOptions);
              ...
              if ($(link).data("view") === sectionView) {
                ...
                $(link).animate(
                  { width: "0px", overflow: "hidden" },
                  {
                    ...
                    complete() {
                      this.remove();
                      // If there's a page order option set on the model, update
                      // it
                      const pageOrder = view.model.get("pageOrder");
                      if (pageOrder && pageOrder.length) {
                        view.updatePageOrder();
                      }
                    },
       ...
       * RETURN or TAB characters, and instead re-directs to updateName(). In
       * the case of the TAB key, the focus moves to the title field.
       * @param {Event} e - The keyup or keydown event when the user types in
       * the portal-section-link field
       */
      limitLabelInput(e) {
        try {
      ...
      updateName(e) {
        ...
        try {
          // Get the PortalSection model for this rename button
          const sectionLink = $(e.target).parents(this.sectionLinkContainer);
          const targetLink = sectionLink.find(this.sectionLinks);
          ...
          // If this section is an object of PortalSection model, update the
          // label.
          if (
            sectionModel &&
            Object.prototype.isPrototypeOf.call(
          ...
          // Update the name set on the link element, since it's used for
          // setting the pageOrder option
          ...
          const indexIDs = this.sectionLabels.indexOf(
            sectionView.uniqueSectionLabel,
          );
          let newUniqueLabel = "";

          // Remove the old unique label so when we create a new unique label,
          // we don't consider the label that we're replacing in determining
          // uniqueness
          if (indexIDs > -1) {
      ...
      /**
       * Using the "section-name" data attribute set on each section link, and
       * the order that the links are displayed in the DOM, update the pageOrder
       * option in the portal model.
       */
      updatePageOrder() {
        try {
          const view = this;
          // Get the links as they are ordered in the UI
          const links = view.el.querySelectorAll(
            view.sectionLinksContainer + view.sortableLinksSelector,
          );
          const pageOrder = [];
          _.each(links, (link) => {
            // Use the value set on section-name to re-order pages
            const label = $(link).data("section-name");
            if (label) {
      ...
      /**
       * Shows a validation error message and adds error styling to the given
       * elements
       * @param {jQuery} elements - The elements to add error styling and
       * messaging to
       */
      ...
      /**
       * This function is called when the app navigates away from this view. Any
       * clean-up or housekeeping happens at this time.
       */
      onClose() {
