  ...
  const ToolbarView = Backbone.View.extend(
    /** @lends ToolbarView.prototype */ {
      ...
      render() {
        ...
        this.sections.forEach((sectionOption) => {
          ...
          const sectionEl = {
            ...
            action,
            sectionView,
            panelId: sectionOption.panelId || null,
          };
      ...
      openPanelById(panelId) {
        ...
        // If restore points to the currently active section, do nothing so we
        // don't toggle the toolbar closed via handleLinkClick.
        if (section?.isActive && this.isOpen) return;
        if (section) {
          this.handleLinkClick(section);
        }
      },
