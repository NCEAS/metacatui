  ...
  var ProvStatementView = Backbone.View.extend(
    /** @lends ProvStatementView */ {
      ...
      render: function () {
          ...
          _.each(predicates, function (p, ii) {
            if (typeof model.get(p) !== "undefined" && model.get(p)) {
              ...
              _.each(predicateValues, function (value, iii) {
                ...
                var modelFromID = _.find(view.relatedModels, function (m) {
                ...
                if (typeof allTriples[inversePredicate] !== "undefined") {
                  while (
                    !hasInverse &&
                    x < allTriples[inversePredicate].length - 1
                  ) {
                    ...
                    if (
                      inverseTriple.subjectID == triple.objectID &&
                      inverseTriple.objectID == triple.subjectID
                    )
                      hasInverse = true;
        ...
        if (this.model.type == "Package") var noun = "package";
        else var noun = this.model.getType();
        ...
        _.each(populatedPredicates, function (predicate, i) {
          ...
          var subjStatementBegin =
              "This " + noun + " " + view.getPredicate(predicate),
            subjList = new Array();

          //State the statement/sentence when this model is the object
          var objStatementBegin =
              "This " + noun + " " + view.getPredicate(predicate, true),
            objList = new Array();
          ...
          _.each(allTriples[predicate], function (triple, ii) {
            ...
            if (
              triple.subject == id ||
              (typeof triple.subject === "object" &&
                triple.subject.get("id") == id)
            ) {
              ...
              var icon = $(document.createElement("i")).attr(
                "class",
                "icon " + view.getIconType(type),
              );

              if (!objectName) {
                if (typeof objectModel !== "undefined")
                  objectName = objectModel.get("id");
                else objectName = type || "";
              }
              ...
              var link = $(document.createElement("a"))
                .attr(
                  "href",
                  "<%= MetacatUI.root %>/view/" + encodeURIComponent(objectId),
                )
                .prepend(linkText)
              ...
              if (
                view.currentlyViewing &&
                view.currentlyViewing.get("id") == objectId
              ) {
                $(linkText)
                  .addClass(
                    "currently-viewing pointer highlight-node node-link",
                  )
                  .attr("data-id", objectId);
                var linkContainer = $(document.createElement("span")).prepend(
                  $(document.createElement("span")).text(
                    "the " +
                      view.currentlyViewing.getType() +
                      " you are currently viewing, ",
                  ),
                  linkText,
                );
                subjList.push(linkContainer);
              } else subjList.push(link);
            }
            //If the object of this triple equals the id of this model, then structure the sentence as so
            else if (
              triple.object == id ||
              (typeof triple.object === "object" &&
                triple.object.get("id") == id)
            ) {
              ...
              if (!subjectName) {
                ...
                else subjectName = type || "";
              }

              var linkText = $(document.createElement("span")).text(
                subjectName,
              );
              ...
              var link = $(document.createElement("a"))
                .attr(
                  "href",
                  "<%= MetacatUI.root %>/view/" + encodeURIComponent(subjectId),
                )
              ...
              if (
                view.currentlyViewing &&
                view.currentlyViewing.get("id") == subjectId
              ) {
                $(linkText)
                  .addClass("currently-viewing pointer highlight-node")
                  .attr("data-id", subjectId);
                var linkContainer = $(document.createElement("span")).prepend(
                  $(document.createElement("span")).text(
                    "the " +
              ...
              } else objList.push(link);
            }
          });

          //Add these statements to our element
          if (subjList.length > 0) {
            var collapseListView = new ExpandCollapseList({
              list: subjList,
              prependText: subjStatementBegin,
              appendText: ". ",
            });
      ...
      getPredicate: function (provFieldName, inverse) {
        ...
        if (provFieldName == "prov_wasDerivedFrom" && !inverse)
          return "was derived from ";
        else if (provFieldName == "prov_wasDerivedFrom" && inverse)
          return "was used as an input to create ";
        else if (provFieldName == "prov_generatedByProgram" && !inverse)
          return "was generated by ";
        else if (provFieldName == "prov_generatedByProgram" && inverse)
          return "generated ";
        else if (provFieldName == "prov_generated" && !inverse)
          return "generated ";
        else if (provFieldName == "prov_generated" && inverse)
          return "was generated by ";
        else if (provFieldName == "prov_usedByProgram" && !inverse)
          return "was used by ";
        else if (provFieldName == "prov_usedByProgram" && inverse)
          return "used ";
      ...
      getIconType: function (type) {
        if (typeof type === "undefined" || !type) return "icon-table";
        else if (type == "program") return "icon-code";
        else if (type == "data") return "icon-table";
        else if (type == "metadata") return "icon-file-text";
        else if (type == "image") return "icon-picture";
        else if (type == "pdf") return "icon-file pdf";
        else if (type == "package") return "icon-folder-open";
      },
