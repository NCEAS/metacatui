  ...
  const SolrResult = Backbone.Model.extend(
    /** @lends SolrResult.prototype */ {
      ...
      handleGetInfoError(error) {
        ...
        if (status === 404) this.set("notFound", true);

        this.set("indexed", false);
        this.trigger("getInfoError", status, message);
      },

      getInfoSuccess(data) {
        // If the Solr response was not as expected, trigger and error and exit
        if (!data?.response?.docs) {
          this.handleGetInfoError({ message: "No data found" });
          return;
        }
      ...
      getSysMeta() {
        ...
        const requestSettings = {
          ...
          error(response) {
            ...
            model.set({ errorStatus, errorMessage, notFound: true });
            model.trigger("error", errorStatus, errorMessage);
          },
        };
