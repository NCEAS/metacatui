  ...
  var QueryField = Backbone.Model.extend(
    /** @lends QueryField.prototype */ {
      ...
      initialize: function (attrs, options) {
        ...
        var isCaseSensitive = this.caseSensitiveTypes().includes(
          this.get("type"),
        );
        if (isCaseSensitive) {
      ...
      descriptions: function () {
        return {
          serviceDescription:
            "A full description of the service that can be used to download the dataset",
          placeKey:
            "A location name, as assigned by the dataset creator. (FGDC metadata only)",
          serviceTitle:
            "A short title of the service that can be used to download the dataset",
          awardNumber:
            "A unique identifier used by a project funder to uniquely identify an award associated with the dataset.",
          funderIdentifier:
            "An identifier that uniquely identifies the organization that funded the dataset.",
          eastBoundCoord:
          ...
          isService:
            "If true, the dataset is available by a download service hosted by the member repository.",
          isPartOf:
            "Include datasets that have been added to this data collection by the portal or dataset owners",
          isPublic:
            "Include or exclude datasets that are available for anyone to view",
          northBoundCoord:
            "Northern most latitude of the spatial extent, in decimal degrees, WGS84",
          replicationAllowed:
            "Only include datasets that are allowed to be replicated to another member repo",
          writePermission: "People or groups who can view and edit the dataset",
          readPermission:
            "People or groups who can view the dataset if it's still unpublished",
          changePermission:
            "People or groups who have been granted complete ownership of the dataset",
          rightsHolder:
            "People or groups who have complete ownership of the dataset",
          ...
          serviceOutput:
            "The data formats available for download from the data service",
          dateUploaded:
            "The date that the content of the dataset was last updated",
          datePublished:
            "The date that the dataset was published, as specified in the metadata.",
          replicaVerifiedDate:
            "The date that the dataset was replicated to another member repository",
          dateModified:
            "The date that the system metadata was last updated (e.g. files renamed, file format changed)",
          fileName: "The file name of the metadata file",
          attribute:
            "The full attribute metadata, which may include a description, label, name, and unit",
          originText:
            "The full name of all people and organizations who are responsible for creating the dataset",
          author:
            "The full name of the dataset creator who is listed first in the metadata",
          authorGivenNameSort:
            "The first name of the dataset creator who is listed first in the metadata",
          authorSurNameSort:
            "The last name of the dataset creator who is listed first in the metadata",
          ...
          investigatorText:
            "The last names of all people responsible for creating the dataset",
          blockedReplicationMN:
            "The member repositories that are blocked from holding replicas of the dataset",
          replicaMN:
            "The member repositories that are holding copies of the dataset",
          preferredReplicationMN:
            "The member repositories that are preferred replication targets of the dataset",
          authoritativeMN:
            "The member repository that currently holds this dataset, which may differ from the repository that the dataset is originally from.",
          datasource:
            "The member repository that originally contributed the dataset.",
          formatId: "The metadata standard or format type",
          contactOrganizationText:
            "The name of all organizations responsible for creating the dataset",
          geoform:
            "The name of the general form in which the item's geospatial data is presented",
          funderName:
            "The name of the organization that funded the project or dataset.",
          purpose:
            "The purpose describes why the dataset was created. (Limited to FGDC metadata only)",
          size: "The size of the metadata file, in bytes.",
          replicationStatus:
            "The status of the DataONE replication process for this dataset. (completed, failed, queued, requested)",
          awardTitle:
            "The title of the award or grant that funded the dataset.",
          serviceType:
            "The type of service that is available to download the dataset from the member repository",
          ...
          serviceEndpoint:
            "The URL of the service that can be used to download the dataset",
          submitter:
            "The username (e.g. ORCID) of the person who submitted or updated the dataset. This may differ from the person responsible for creating the data.",
          westBoundCoord:
          ...
          prov_hasSources:
            "One or more identifiers of metadata documents that have been used as sources to create a derived dataset. The derived datasets will be included in the search results.",
          prov_hasDerivations:
            "One or more identifiers of metadata documents that have been derived from another dataset. The source datasets will be included in the search results.",
          indexeddate:
            "The date that the metadata document was last indexed (or re-indexed) in the search system.",
        };
      },
      ...
      categoriesMap: function () {
        return [
          ...
          },
          {
            ...
            icon: "archive",
            queryFields: ["authoritativeMN", "datasource"],
          },
          {
            label: "Permissions",
            icon: "lock",
          ...
          },
          {
            label: "Identifier",
            icon: "tag",
            queryFields: ["identifier", "resourceMap", "documents", "seriesId"],
          },
          {
      ...
      getReadableName: function () {
        try {
          ...
          console.log(
            "Failed to create a readable name for a Query Field, error message: " +
              e,
          );
        }
      },
      ...
      getCategory: function () {
        try {
          ...
          console.log(
            "Failed to categorize a Query Field, error message: " + e,
          );
        }
      },
      ...
      getFilterType: function () {
        try {
          ...
          console.log(
            "Failed to find a matching filter type for a Query Field, error message: " +
              e,
          );
        }
      },
      ...
      isType: function (type) {
        try {
          ...
          console.log(
            "Failed to check if query field is a type, error message: " + e,
          );
        }
      },
