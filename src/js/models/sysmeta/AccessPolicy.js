define([
  ...
  /**
   * Normalize an input value to a positive integer and ensure it is within the
   * valid range of indices. Throws an error if the index is out of bounds.
   * @param {*} index Value to normalize as an array index.
   * @param {Integer} length The length of the array to validate the index against.
   * @returns {number} The normalized array index.
   */
  const requireIndex = (index, length) => {
    const normalizedIndex = requireNonNegativeInteger(index);
    if (normalizedIndex >= length) {
      throw new Error(
        `AccessPolicy operation could not find rule at index ${normalizedIndex}.`,
      );
    }
  ...
  class AccessPolicy extends Array {
     ...
     */
    replace(index, rule) {
      const normalizedIndex = requireIndex(index, this.length);
    ...
    /** @returns {Array<AccessRule>} All access rules in the policy. */
    getRules() {
      return Array.from(this);
    }

    /** @returns {Array<AccessRule>} Rules missing a subject or permissions. */
    getEmptyRules() {
      return this.getRules().filter((rule) => rule.isEmpty());
    }

    /** @returns {Array<AccessRule>} Rules with a subject and permissions. */
    getNonEmptyRules() {
      return this.getRules().filter((rule) => !rule.isEmpty());
    }

    /** Remove rules missing a subject or permissions. */
    removeEmptyRules() {
      this.getEmptyRules().forEach((rule) => this.remove(this.indexOf(rule)));
    }
    ...
    validate(path = "accessPolicy") {
      const errors = [];
      this.getNonEmptyRules().forEach((rule, index) => {
     ...
     */
    toElement(doc) {
      const rules = this.getNonEmptyRules();
     ...
     */
    toJSON() {
      return this.getNonEmptyRules().map((rule) => rule.toJSON());
    }
