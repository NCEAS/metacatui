...
define(["uriTemplatesEs"], ({ UriTemplate }) => {
  ...
  const parseTemplate = (template) => {
    ...
    return {
      ...
      // Keep a compatible shape for existing callers. Expression internals are
      // intentionally opaque and managed by uri-templates-es.
      expressions: [],
      varNames,
    };
  };
  ...
  const extractValuesFromUrl = (template, urlString) => {
    ...
    const parsed = instance.fromUri(urlString);
