...
if(isMCUI&&!inIframe){block.allMenuLinks.forEach((link,i)=>{link.el.href=link.el.href.replace(urlMCUI,"");})}}
...
showMenu(block.profile);hideMenu(block.signin);block.profileLinkEls.forEach((link,i)=>{const baseProfileUrl=(isMCUI&&!inWebsiteIframe)?"":urlMCUI;link.el.href=link.el.href.replace(/USERNAME/,data.username).replace(/^.*?SEARCH/,baseProfileUrl);if(link.el.title.toLowerCase()==="my portals"){if(!data.showPortals){link.el.style.display="none"}}});}}
